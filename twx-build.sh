...
extract(){
    ...
    if [ -n "$check_dockerfiles" ]; then
        echo -e "\033[32;1m[✓]\033[0m The following Dockerfiles have been found: \033[33;1m$check_dockerfiles\033[0m"
        tar -xzf $check_dockerfiles
        echo -e "\033[32;1m[✓]\033[0m Dockerfiles have already been extracted."
    else
        echo -e "\033[31;1m[✗]\033[0m No Dockerfiles were found."
        exit
    fi
}
...
setup_twx_postgres(){
    ...
    if [ -n "$check_xmlmerge" ]; then echo -e "\033[32;1m[✓]\033[0m The XMLmerge file has been found: \033[33;1m$check_xmlmerge\033[0m"; else echo -e "\033[31;1m[✗]\033[0m The XMLmerge file has not been found."; exit; fi
...
build_twx_postgres(){
    echo -e "\033[32;1m[✓]\033[0m Initiating the build process for the ThingWorx PostgreSQL Docker images."
    file_env="build.env"
...
build_twx_mssql(){
    echo -e "\033[32;1m[✓]\033[0m Initiating the build process for the ThingWorx MSSQL Docker images."
    file_env="build.env"
    check_base_image_name=$(grep "BASE_IMAGE=" "$file_env" | cut -d'=' -f2)
    sudo docker pull $check_base_image_name
